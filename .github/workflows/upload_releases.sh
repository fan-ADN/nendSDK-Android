...
VERSION=$1
API_RELEASES_LIST="https://api.github.com/repos/fan-ADN/nendSDK-Android/releases"
UPLOAD_URL=''
...
function parseUploadUrlFromResponse() {
    ...
    do
        if [ "`echo $line | grep "upload_url"`" ]; then
            ...
            UPLOAD_URL=`echo ${UPLOAD_URL//\{?name,label\}/?name=apks.zip}`
...
curl -X POST -H "Authorization: token $TOKEN" \
    -H "Content-Type: application/zip" --data-binary @apks.zip $UPLOAD_URL
